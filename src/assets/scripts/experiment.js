    ...
    updateUniforms() {
        if (this.interactive && this.shouldCompleteRip()) {
            ...
            this.sides.forEach((side) => {
                ...
                uniforms.uTearXAngle.value =
                    this.sheetSettings[side.id].tearXAngle;
                uniforms.uTearYAngle.value =
                    this.sheetSettings[side.id].tearYAngle;
                uniforms.uTearZAngle.value =
                    this.sheetSettings[side.id].tearZAngle;
                uniforms.uTearXOffset.value =
                    this.sheetSettings[side.id].tearXOffset;
                uniforms.uXDirection.value =
                    this.sheetSettings[side.id].direction;
                uniforms.uShadeColor.value =
                    this.sheetSettings[side.id].shadeColor;
                uniforms.uShadeAmount.value =
                    this.sheetSettings[side.id].shadeAmount;
...
if (canvas) {
    stage = new Stage(canvas, 'transparent', 'transparent');
    ...
    const images = [
        {
            texture: textureLoader.load(temple),
            colors: [
                [192, 208, 220],
                [217, 190, 174],
            ],
        ...
        {
            texture: textureLoader.load(market),
            colors: [
                [168, 163, 150],
                [218, 180, 141],
            ],
    ...
    const addNewPhoto = () => {
        ...
        if (images.length - currentImage < 2 && extraImages.length)
            loadExtraPhoto();
